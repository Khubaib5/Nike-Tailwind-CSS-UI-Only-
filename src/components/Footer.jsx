  ...
  return (
    <footer className='max-container'>
      <div className='flex justify-between items-start gap-20 flex-wrap max-lg:flex-col'>
        ...
        <div className='flex items-center gap-5 mt-8 max-sm:flex-col'>
          {
          ...
          <br />
          <div className='flex flex-1 justify-between lg:gap-10 g-20 flex-wrap max-sm:justify-between'>
          {
          footerLinks.map((section, index)=> (
            <div key={section.id} className='hover:text-red-700 max-sm:flex max-sm:flex-col'>
              <h4 className='text-white max-sm:mr-7 max-sm:mt-8 max-sm:font-bold max-sm:text-2xl'>
                {section.title}
              </h4>
              <ul>
                {section.links.map((link)=>(
                  <li key={link.title} className='mt-3 text-white-400 font-montserrat text-base leading-normal cursor-pointer hover:text-red-500 max-sm:flex max-sm:flex-col'>
